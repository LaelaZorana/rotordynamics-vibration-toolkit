...
# fix the y plane so the eigenproblem is planar and non degenerate: the
# isotropic rotor has repeated x and y pairs whose eigenvectors come back in
# an arbitrary basis, so mode shapes must be taken from a single plane
y_dofs = [shaft.dof(n, kind) for n in range(shaft.n_nodes) for kind in ("y", "ys")]
w, modes = shaft.undamped_frequencies(n_modes=3, fixed_dofs=y_dofs)
plot_mode_shapes(shaft.z, modes, w, n=3).savefig(os.path.join(OUT, "mode_shapes.png"), dpi=150)
